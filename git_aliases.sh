...
gitimpacttotal()
{
    author="$1"
    leftovers="${*:2}"
    res=`/usr/bin/git log --numstat --pretty="%H" --author="$author" $leftovers | awk 'NF==3 {plus+=$1; minus+=$2} END {printf("+%d, -%d\n", plus, minus)}'`
...
gitimpactpy()
{
    ...
    res=`/usr/bin/git log --numstat --pretty="%H" --author=$author $leftovers -- \*.py| awk 'NF==3 {plus+=$1; minus+=$2} END {printf("+%d, -%d\n", plus, minus)}'`
    added=`echo $res | cut -d, -f1 | cut -d+ -f2`
    removed=`echo $res | cut -d, -f2 | cut -d- -f2`
    net=`expr $added - $removed`
    echo "Net: $net (Added: $added Removed: $removed)"
